...
function msgHTML() {
  ...
  var html = '<div class="sec-h">APRS Text Messaging ' + badge + '</div>'
  ...
  if (msgError) {
    html += '<div style="font-size:11px;color:var(--red);margin-top:6px">Error: ' + msgError + '</div>';
  }
  ...
  html += '<div class="tool-divider"></div>'
    + '<div class="sec-h">Messages (' + msgList.length + ')</div>';

  if (!msgList.length) {
    html += '<div class="empty">No messages yet.' + (msgConnected ? '' : '<br>Connect to APRS-IS to send and receive.') + '</div>';
  } else {
